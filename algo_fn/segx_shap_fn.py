...
def segx_gradient_shap_fn(model, seg_model, val_dataset, test_dataset, config: GradientSHAPConfig):
    """
    ...
    for idx, (image, label, image_name) in enumerate(test_dataset):
        ...
        for idx, shap_map in shap_map_dict.items():
            shap_map = shap_map*seg_mask
            saliency_map = get_saliency_map(shap_map, top_n_percent=5)
